...
<cfif url.action eq 'save'>
    ...
    <cfelse>
		<cfquery datasource="#request.dsn#" name="qUpdate">
            ...
            values(
            	<cfif session.isAdmin>
            		'#session.userQuery.username#', 
                <cfelse>
                	'#form.username#',
                </cfif>
                '#form.justification#', 
                '#form.AvailabilitySuitability#',
                '#form.ReagentList#',
                '#form.BrainExpression#',
                <cfif session.isAdmin>
	                '#form.Status#',
                </cfif>
...
<p>Thank your for suggesting a new Neuromab using our online form. The following suggestion has been received:</p>
            ...
            <td>
                ...
                <cfgrid name="suggestions" format="html">
                    ...
                    <cfloop query="qSuggestions">
                        ...
                        <cfgridrow data="#replace(qSuggestions.Justification, ',', '&##44;', 'ALL')# ,#replace(qSuggestions.AvailabilitySuitability, ',', '&##44;', 'ALL')# ,#replace(qSuggestions.ReagentList, ',', '&##44;', 'ALL')# ,#replace(qSuggestions.BrainExpression, ',', '&##44;', 'ALL')# ,#replace(qSuggestions.status, ',', '&##44;', 'ALL')# ,#replace(editVal, ',', '&##44;', 'ALL')#,#replace(deleteVal, ',', '&##44;', 'ALL')#,#replace(emailVal, ',', '&##44;', 'ALL')#">
